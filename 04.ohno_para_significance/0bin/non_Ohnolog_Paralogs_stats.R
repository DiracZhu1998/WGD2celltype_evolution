...
paralogs <- paralogs[!paralogs$Gene.name %in% unique(c(ohnologs$Symbol1, ohnologs$Symbol2)),]
paralogs <- paralogs[!paralogs$paralogue.associated.gene.name %in% unique(c(ohnologs$Symbol1, ohnologs$Symbol2)),]
...
p0 <- ggplot(data = NULL, aes(x = sapply(paralog_pairs, function(x) length(x) ))) +
  ...
  labs(title = "Distribution of the number of non-ohnolog paralog \n in non-ohnolog paralog families") +
  theme(plot.title = element_text(hjust = 0.5))
ggsave(paste0(args$out, '/', species,".whole_non-ohnolog-paralog_family_size.pdf"), p0, width = 8, height = 5)


##### non-ohnolog paralog ratio within DEGs #########
...
p1 <- stats %>% as.data.frame() %>% ggplot(aes(`paralogs%`)) +
  ...
  geom_vline(aes(xintercept=paralog_ratio_in_ourdata),
             color="red", linetype="dashed", linewidth=1) + xlim(0,1) + xlab("non-ohnolog paralogs %")
ggsave(paste0(args$out, '/', species,".non-ohnolog-paralog_ratio_inDEGs.pdf"), p1, width = 6, height = 2)

### get stats of non-ohnolog paralogs  ###
...
p2 <- stats %>% as.data.frame() %>% ggplot(aes(`families_divided_by_paralogs%`)) +
  ...
  geom_vline(aes(xintercept=length(paralog_pairs_in_our_dataset)/length(unlist(paralog_pairs_in_our_dataset))), # you should use
             color="red", linetype="dashed", linewidth=1) + xlim(0,1) + xlab("non-ohnolog paralog family divided by non-oh paralogs")

ggsave(paste0(args$out, '/', species,".non-ohnolog-paralog_families_divided_non-ohnolog-paralogs_inDEGs.pdf"), p2, width = 6, height = 2)
...
venn.plot <- venn.diagram(
  ...
  category.names = c("DEG", "non-ohnolog paralogs"),
  filename = paste0(args$out, '/', species,".DEG_non-ohnolog-paralog.Venn.png"),
  col=c("#440154ff", '#21908dff'),
  fill = c(alpha("#440154ff",0.3), alpha('#21908dff',0.3)),
  output = TRUE
)
############## Chi squared test of non-ohnolog paralogs and DEGs ##################
...
colnames(chi) <- c("non-oh paralogs", "others")

write.table(x = chi, file = paste0(args$out, '/', species, ".non-ohnolog-paralog_DEGs.chi2.txt"), sep = "\t", quote = F,
            col.names = T, row.names = T)
write.table(x = capture.output(print(chisq.test(chi, correct = F)))[5], 
            file = paste0(args$out, '/', species, ".non-ohnolog-paralog_DEGs.chi2.txt"), sep = "\t", quote = F,
            col.names = F, row.names = F, append = T)
write(paste0("OR = ",(chi[1,1]*chi[2,2])/(chi[1,2]*chi[2,1])), file = paste0(args$out, '/', species, ".non-ohnolog-paralog_DEGs.chi2.txt"), sep = "\t", append = T)
