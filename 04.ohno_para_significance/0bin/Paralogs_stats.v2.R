...
stats <- subcluster_markers %>% as_tibble() %>% group_by(cluster) %>% 
  summarise(total = length(cluster), 
    n_paralogs = sum(gene %in% unique(c(paralogs$Dup1, paralogs$Dup2))), # calculate number of paralogs
    n_paralog_pairs = length(unique(vapply(gene[gene %in% unique(c(paralogs$Dup1, paralogs$Dup2))], 
...
write.table(stats, file = paste0(args$out, '/', species, '.paralog_ratio_inDEGs.stats.txt'), col.names = T, quote = F, sep = '\t', row.names = F)

paralog_number_in_ourdata <- sum(unique(c(paralogs$Dup1, paralogs$Dup2)) %in%  bg)
paralog_ratio_in_ourdata <- round(paralog_number_in_ourdata/length(bg),2)
non_paralog_number_in_ourdata <- sum(!bg %in% unique(c(paralogs$Dup1, paralogs$Dup2)))
...
annotate("text", x = paralog_ratio_in_ourdata, y = 1, color = "red", label = paralog_ratio_in_ourdata)

ggsave(paste0(args$out, '/', species,".paralog_ratio_inDEGs.pdf"), p1, width = 6, height = 2)
write(paste0("paralogs\t", paralog_ratio_in_ourdata), file = paste0(args$out, '/', species, '.ratio_bg.txt'), append = T)

### get stats of paralogs of one-one-one orthologs ###
paralogs_in_our_dataset <- paralogs[paralogs$Dup1 %in% bg & paralogs$Dup2 %in% bg, ]
g <- graph_from_data_frame(paralogs_in_our_dataset[,c(1,2)], directed = FALSE)
components <- clusters(g)$membership
...
p2 <- stats %>% as.data.frame() %>% ggplot(aes(`families_divided_by_paralogs%`)) +
  geom_density(fill="#69b3a2", color="#e9ecef", alpha=0.6) +
  geom_vline(aes(xintercept=mean(`families_divided_by_paralogs%`, na.rm = T)),
             color="blue", linetype="dashed", linewidth=1) +
...
write(paste0("paralogs\t", length(paralog_pairs_in_our_dataset)/paralog_number_in_ourdata), file = paste0(args$out, '/', species, '.family_ratio_bg.txt'), append = T)

######### Label all genes with different DEGs and paralogs as T/F #########
stats_for_chi2 <- data.frame(gene = bg, DEG = NA, paralog = NA)
stats_for_chi2$DEG <- stats_for_chi2$gene %in% subcluster_markers$gene
stats_for_chi2$paralog <- stats_for_chi2$gene %in% unique(c(paralogs$Dup1, paralogs$Dup2))
...
write.table(x = chi, file = paste0(args$out, '/', species, ".paralog_DEGs.fisher.txt"), sep = "\t", quote = F,
            col.names = T, row.names = T)
write.table(x = capture.output(print(fisher.test(chi)))[5], 
      file = paste0(args$out, '/', species, ".paralog_DEGs.fisher.txt"), sep = "\t", quote = F,
      col.names = F, row.names = F, append = T)
write(paste0("OR = ",(chi[1,1]*chi[2,2])/(chi[1,2]*chi[2,1])), file = paste0(args$out, '/', species, ".paralog_DEGs.fisher.txt"), sep = "\t", append = T)


# OR at cell type level
OR_celltype <- data.frame()
for (c in unique(stats$cluster)){
    stats_for_chi2 <- data.frame(gene = bg, DEG = NA, paralog = NA) # use all bg might not be good as using cell type bg
...
write.table(x = OR_celltype, file = paste0(args$out, '/', species, ".paralog_DEGs.fisher.celltype.txt"), sep = "\t", quote = F,
