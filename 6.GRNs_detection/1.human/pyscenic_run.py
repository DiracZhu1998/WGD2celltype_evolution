...
TFS_FNAME = os.path.join(RESOURCES_FOLDERNAME, 'TFs_Ensembl.txt')
RANKING_DBS_FNAMES = list(map(lambda fn: os.path.join(RESOURCES_FOLDERNAME, "cisTargets/", fn),
    ['hg38-500bp.feather','hg38-tss-centered-10kb.feather']))
print(RANKING_DBS_FNAMES)
MOTIF_ANNOTATIONS_FNAME = os.path.join(RESOURCES_FOLDERNAME, 'Ensembl.motifs-v9-nr.hgnc-m0.001-o0.0.tbl')
CELL_ANNOTATIONS_FNAME = os.path.join(MAIN, "0.cellInfo.subset.csv")
#EXP_MTX_TPM_FNAME = os.path.join(MAIN, '0.LogexprMat.subset.csv')
EXP_MTX_COUNTS_FNAME = os.path.join(MAIN, '0.exprMat.subset.csv')

# Results file name
DATASET_ID = "Human_brain"
...
minCountsPerGene = 1 * 0.01 * df_umi.shape[1] # equal to at least 1% cells express that gene with 3UMI or 3% cells express that gene with 1 UMI # AIF was filtered in previous settings
minSamples = df_umi.shape[1] * 0.005 # at least 0.5 % cells express
...
df_obs = df_metadata[['CellID', 'Tissue', 'donor_id', 'dissection', 'supercluster', 'Refined family', 'Refined subtype']].set_index('CellID').sort_index()
adata.obs = df_obs
