...
library(Seurat)
library(dplyr)
gene_info <- read.delim("0.mouse_gene_info_from_Ensembl.mod", header =T, sep = "\t")
feather1 <- read_feather("/mnt/data01/yuanzhen/01.Vertebrate_cell_evo/09.GRNs_detection/1.mouse/0.bin/cisTargets/mm9-500bp-upstream-7species.mc9nr.feather")
feather2 <- read_feather("/mnt/data01/yuanzhen/01.Vertebrate_cell_evo/09.GRNs_detection/1.mouse/0.bin/cisTargets/mm9-tss-centered-10kb-7species.mc9nr.feather")
...
duplicated <- table(colnames(feather1))
duplicated <- names(duplicated)[duplicated >= 2]
for (i in duplicated){
    tmp <- which(colnames(feather1) == i)
    feather1[, tmp[2:length(tmp)]] <- NULL #only keep first occurennce
    feather2[, tmp[2:length(tmp)]] <- NULL
}

#write_feather(feather1, sink = "/mnt/data01/yuanzhen/01.Vertebrate_cell_evo/09.GRNs_detection/1.mouse/0.bin/cisTargets/Ensembl.mm9-500bp-upstream-7species.mc9nr.feather", version = 1)
#write_feather(feather2, sink = "/mnt/data01/yuanzhen/01.Vertebrate_cell_evo/09.GRNs_detection/1.mouse/0.bin/cisTargets/Ensembl.mm9-tss-centered-10kb-7species.mc9nr.feather", version = 1)

atlas <- readRDS("/mnt/data01/yuanzhen/01.Vertebrate_cell_evo/01.data/02.atlas/mouse/mouse_atlas_test_onlybrain.rds")
...
cellInfo$CellType <- cellInfo$TaxonomyRank3
